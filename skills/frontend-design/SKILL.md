...
A real photo or a well-built interaction pattern reads as more crafted than a gradient rectangle or a hand-rolled toggle that almost works. When the brief calls for photography, icons, or a standard interaction pattern (toggle, pricing card, loader, accordion), pull real material with your fetch/browser tools instead of inventing a placeholder or guessing at markup from memory.

- **Photography — Unsplash** (unsplash.com). Search for the specific subject in the brief, not generic stock imagery ("wet basalt countertop" beats "coffee shop interior"). Pull actual `images.unsplash.com` URLs so the page renders real photos, and credit the photographer in an unobtrusive footer credit line where license terms call for it.
- **Interaction patterns — uiverse.io**. A library of free, community-built HTML/CSS/Tailwind components — buttons, loaders, toggles, cards. Treat it as reference for *how the interaction works*, not as a finished look: pull the markup/CSS, then restrip and restyle it with your own token system (palette, type, radii, easing) before it goes on the page. A raw uiverse component dropped next to hand-tuned typography is the fastest way to make a page look assembled instead of designed.
- **Component/icon reference — Untitled UI** (untitledui.com). Professionally designed React/Figma components and a free icon set. Useful for checking how a well-designed system handles a pattern (form validation states, empty states, nav structure) — again, reimplement in your own token system rather than importing its default look wholesale, or the page will read as Untitled UI's brand instead of this one.
